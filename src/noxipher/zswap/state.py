...
class MerkleTree:
    ...
    def proof(self, index: int) -> list[bytes]:
        ...
        tree_nodes = {level: {} for level in range(self._depth + 1)}
        ...
        for level in range(self._depth):
            for idx in list(tree_nodes[level].keys()):
                ...
                if p_idx not in tree_nodes[level + 1]:
                    left_idx = p_idx * 2
                    right_idx = left_idx + 1
                    left = tree_nodes[level].get(left_idx, Fr.from_le_bytes(self._zero_nodes[level]))
                    right = tree_nodes[level].get(right_idx, Fr.from_le_bytes(self._zero_nodes[level]))
        ...
        for level in range(self._depth):
            ...
            sibling_val = tree_nodes[level].get(sibling_idx, Fr.from_le_bytes(self._zero_nodes[level]))
